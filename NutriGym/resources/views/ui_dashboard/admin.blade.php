...
@if(session('success'))
<div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-4 rounded shadow-sm flex justify-between items-center" id="alert-success">
    <div>
        <p class="font-bold">¡Éxito!</p>
        <p>{{ session('success') }}</p>
    </div>
    <button onclick="document.getElementById('alert-success').style.display='none'" class="text-green-700 font-bold hover:text-green-900">&times;</button>
</div>
@endif
...
<script>
    // NUEVO: Traemos los roles disponibles desde PHP a JavaScript
        ...
        document.getElementById('detallesUsuario').innerHTML = `
            <div class="bg-white p-6 rounded-lg shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-600 mb-1">Cambiar Rol</label>
                        <form action="/admin/usuarios/${usuario.id}/rol" method="POST">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type="hidden" name="_method" value="PUT">
                            <select name="id_rol" onchange="this.form.submit()" class="input-neu w-full p-2 bg-gray-50 text-sm">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-600 mt-2">Fecha de Nacimiento</label>
                        <p class="text-lg text-gray-800">${new Date(usuario.fecha_nacimiento).toLocaleDateString()}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600 mt-2">Fecha de Registro</label>
                        <p class="text-lg text-gray-800">${new Date(usuario.fecha_registro).toLocaleDateString()}</p>
                    </div>
                </div>
                
                <div class="flex flex-col sm:flex-row flex-wrap gap-3 mt-6 border-t pt-4">

                    ${
                        usuario.id_rol == 4 
                        ? `
                            <button onclick="openObjetivoModal()" 
                                    class="btn-neu bg-gray-800 hover:bg-gray-900 text-white w-full sm:w-auto text-sm">
                                Ver objetivos
                            </button>

                            <button onclick="openPreferenciaModal()" 
                                    class="btn-neu bg-red-500 hover:bg-red-600 text-white w-full sm:w-auto text-sm">
                                Ver Preferencias
                            </button>
                        ...
                        `
                        : `
                            <button class="btn-neu bg-gray-300 text-gray-600 cursor-not-allowed w-full sm:w-auto text-sm" disabled>
                                Este rol no tiene opciones médicas
                            </button>
                        `
                    }

                    <button class="btn-neu bg-red-600 hover:bg-red-700 text-white w-full sm:w-auto text-sm ml-auto">
                        Eliminar Usuario
                    </button>
                ...
                <div id="progresoContent" class="space-y-4 sm:space-y-6">
                    ...
                    <div id="progresoData" class="hidden space-y-4 sm:space-y-6">
                        ...
                        <div class="neumorphic-inset p-3 sm:p-4 rounded-lg">
                            ...
                            <div id="pesoChart" class="h-32 sm:h-40 flex items-end justify-between space-x-1"></div>
                        </div>
                        <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="neumorphic-inset p-3 sm:p-4 rounded-lg">
                                ...
                                <div id="estadoInicial" class="text-xs sm:text-sm text-gray-600"></div>
                            </div>
                            <div class="neumorphic-inset p-3 sm:p-4 rounded-lg">
                                <h4 class="font-semibold text-gray-700 mb-2 text-sm sm:text-base">Estado Actual</h4>
                                <div id="estadoActual" class="text-xs sm:text-sm text-gray-600"></div>
                            </div>
                        </div>
                    </div>
